...
shinyServer(function(input, output, session) {
  ...
  # input <- list(age=70,vascular_disease="No",ckd="Yes")
  QQ <- reactive({
  ...
  output$Dynamic <- renderUI({
    ...
    LL[[2]] <- selectInput(inputId="vascular_disease", 
                           ...
                           choices=c("Yes","No"),
                           selected="No")
    LL[[3]] <- selectInput(inputId="ckd", 
                           label="eGFR<60 with eGFR estimated using the MDRD method?",
                           choices=c("Yes","No"),
                           selected="No")
    return(LL) 
  })
  ...
  output$plot <- renderPlot({
    ...
    ggplot(data=x2()) +
      ...
      scale_y_continuous( breaks = seq(0, 0.1, by = 0.01), limits = c(0,0.1), labels = percent) +
